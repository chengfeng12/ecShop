    ...
    <div class="shuang">
      <div class="sl-item" v-if="colorGroup.length == 0">
        ...
        <div class="d-detail">
          ...
          <div class="subtitle line-hide" v-if="goods.includes('goods_subtitle')">这里是商品副标题</div>
          <div style="display: flex;">
            <div style="flex: 1 1 0%;">
              <div class="price">
                <span class="text" v-if="goods.includes('goods_price')">
                  ￥
                  <span>20.00</span>
                </span>
                <span class="productprice" v-if="goods.includes('origin_price')">￥0.00</span>
              </div>
              <div class="sold" v-if="goods.includes('goods_sale')">
                <span>已售0</span>
              </div>
            </div>
      ...
      <div v-else class="sl-item" v-for="(item,index) in colorGroup" :key="index">
        ...
        <div class="d-detail">
          <div
            ...
            v-if="goods.includes('goods_title')"
          >{{item.title || '这里是商品标题'}}</div>
          <div
            class="subtitle line-hide"
            v-if="goods.includes('goods_subtitle')"
          >{{item.subTitle || '这里是商品副标题'}}</div>
          <div style="display: flex;">
            <div style="flex: 1 1 0%;">
              <div class="price">
                <span class="text" v-if="goods.includes('goods_price')">
                  ￥
                  <span>{{item.price || '20.00'}}</span>
                </span>
                <span class="productprice" v-if="goods.includes('origin_price')">￥0.00</span>
              </div>
              <div class="sold" v-if="goods.includes('goods_sale')">
                <span>已售{{item.isSales || 0}}</span>
              </div>
            </div>
            <span class="buy buybtn1" v-if="cart == 'cart_show'">购买</span>
          </div>
