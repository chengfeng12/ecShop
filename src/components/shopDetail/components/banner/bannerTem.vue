...
<template>
  <div class="banner diyitem" :style="{height:dot_position == 'dot_inside' ? '210px' : '240px'}">
    ...
    <div class="dots" v-if="dot_isshow == 'dot_show'">
      <div :class="[dot_type]" :style="{background:index == currentId ? dot_color : 'rgba(0,0,0,0.2)',margin:'0px 6px'}" v-for="(item,index) in bannerLists" :key="index"></div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    mounted() {
      this.$nextTick(function () {
        let _this = this
        console.log(_this.$refs.carousel)
        _this.$refs.carousel.handleResize()
