        ...
        <div class="specificationtable">
          ...
          <CheckboxGroup v-model="specsId" @on-change="selectIdfuc($event)">
            <div class="specificationtbody-item" v-for="(item,index) in specificationData" :key="index">
              ...
              <div>{{item.price}}</div>
              <div>
                <Input v-model="item.mealprice" placeholder="请输入套餐售价" @on-change="changeMeal(index)"/>
              </div>
              <div>
                <Checkbox
                  size="large"
...
export default {
  ...
  methods: {
    ...
    changeMeal(index){
      let currentItem = this.specificationData[index]
      let currentMeal = this.currentMeal

      if(currentMeal.length == 0){
        currentMeal.push({
          id:currentItem.id,
          mealprice:currentItem.mealprice
        })
      }else{
        let key = false,mealid,mealindex
        for (let i = 0; i < currentMeal.length; i++) {
          if(currentMeal[i].id == currentItem.id){
            key = true
            mealid = currentMeal[i].id,
            mealindex = i
          }
        }
        if(key){
          currentMeal[mealindex].mealprice = currentItem.mealprice
        }else{
          currentMeal.push({
            id:currentItem.id,
            mealprice:currentItem.mealprice
    ...
    selectIdfuc: function(data) {
      ...
      for(let i in this.specificationData){
        console.log(this.selectdata,this.specificationData[i].id)
        if(this.selectdata.indexOf(this.specificationData[i].id)!=-1){
          console.log(this.specificationData[i])
          arr.push(this.specificationData[i].title)
          mealArr.push(this.specificationData[i].mealprice)
        } 
      }
      this.selectTitle = arr
      this.selectMeal = mealArr
    ...
    selectSpecsId: function() {
      ...
      this.$set(this.containerData[this.IDindex],'mealprice',this.selectMeal)
    ...
    selectSpecification: function(data) {    
      ...
      console.log('当前的商品数据--------------',data,this.currentMeal)
      ...
      .then(res => {
        if (res.code == 1) {
            ...
            this.specificationData.map((v,i) => {
              console.log(v.id)
              this.currentMeal.map((vi,ii) => {
                console.log(vi.id)
                if(v.id == vi.id){
                  console.log('.........尹宝宝')
                  this.specificationData[i].mealprice = vi.mealprice
                }
              })
            })
    ...
    selGoodsIds: function(data) {
      ...
      data.map((v,i) => {
        ...
        if(v.specIds){
          let newSpec = v.specIds.split(',')
          let newMeal = v.mealprices.split(',')
          newSpec.map((vi,ii) => {
            let newObj = {
              id:vi,
              mealprice:newMeal[ii]
            }

            currentMeal.push(newObj)
          })
      ...
      let idarr=[]
      for(let i in data){
        idarr.push(this.optionss[i].goodsId)
      }
    ...
    getSpecIds(data,editAry){
      ...
      data.map((v,i) => {
        ...
        .then(res => {
          ...
          let mealItem = goodsIdItem.mealprices
          console.log('asidjhfjhhasdkljhflkjahsdkjhlfasdf----',goodsIdItem,mealItem)
          ...
          if(res.code == 1){
            ...
            newData = {
              ...
              mealprice:mealItem ? mealItem.split(',') : []
            }
  ...
  watch: {
    ...
    optionss: function() {
      console.log("父ID",this.optionss);

      this.selGoodsIds(this.optionss);
    }, 
  }
};
</script>
<style scoped>
@import "./couponAddgoodPrice.css";
</style>
