...
<template>
  <div class="container">
    <div class="redu-box">
          ...
          <Form :label-width="100" ref="reduForm" :model="reduForm" :rules="reduRule"
            ...
            <div class="inset-content">
              ...
              <FormItem label="活动时间" prop="time">
                <DatePicker :options="optionsTime" type="datetimerange" v-model="reduForm.timess"
            ...
            <div class="inset-content">
              ...
              <FormItem label="会员等级" v-if="reduForm.activityObject=='3'" prop="member">
                <Select style="width:300px" placeholder="请选择会员等级">
                  <Option v-for="item in reduForm.grade" :value="item.value" :key="item.index">{{item.label}}</Option>
                </Select>
              </FormItem>
              ...
              <FormItem label="优惠设置" v-if="reduForm.type == '1'">
                <div v-for="(item,index) in cusAry" :key="index">
                  <div class="set">
                    优惠{{index+1}}
                    <span class="star">*</span>
                    消费满&nbsp;&nbsp;
                    <Input @blur.native.capture="testCusAry(index)" v-model="item.consumptionAmount"
                      style="width: 100px;" type="number"></Input>元 &nbsp;&nbsp;
                    优惠金额&nbsp;&nbsp;
                    <Input @blur.native.capture="testPrice(index)" v-model="item.couponAmount" style="width: 100px;"
                      type="number"></Input>元
              ...
              <FormItem label="优惠设置" v-if="reduForm.type == '2'">
                <div v-for="(item,index) in cusAry2" :key="index">
                  <div class="set">
                    优惠{{index+1}}
                    <span class="star">*</span>
                    消费满&nbsp;&nbsp;
                    ...
                    优惠金额&nbsp;&nbsp;
                    <Input @blur.native.capture="testPrice2(index)" v-model="item.couponAmount" style="width: 100px;"
                      type="number"></Input>元
      ...
      <!--保存 取消 -->
      <div class="formBtn">
        <Button type="primary" :loading="loading" class="comBtn" @click.native="save(reduForm)">
          保存     
        </Button>
    ...
    methods: {
      ...
      testCusAry(index) {
        ...
        if (this.cusAry[index].consumptionAmount.indexOf(".") != -1) {
          this.cusAry[index].consumptionAmount = Number(
            this.cusAry[index].consumptionAmount
          ).toFixed(2);
        }
      ...
      testCusAry2(index) {
        ...
        if (this.cusAry2[index].consumptionAmount.indexOf(".") != -1) {
          this.cusAry2[index].consumptionAmount = Number(
            this.cusAry2[index].consumptionAmount
          ).toFixed(0);
        }
      ...
      testPrice(index) {
        ...
        if (this.cusAry[index].couponAmount.indexOf(".") != -1) {
          this.cusAry[index].couponAmount = Number(
            this.cusAry[index].couponAmount
          ).toFixed(2);
        }
      },
      testPrice2(index) {
        ...
        if (this.cusAry2[index].couponAmount.indexOf(".") != -1) {
          this.cusAry2[index].couponAmount = Number(
            this.cusAry2[index].couponAmount
          ).toFixed(2);
        }
      },
        ...
        marking
          ...
          .then(res => {
            if (res.code == 1) {
              ...
              this.reduForm.consumptionAmount = res.data.consumptionAmount;
              this.reduForm.couponAmount = res.data.couponAmount;
              this.reduForm.expiryBeginTime = res.data.expiryBeginTime;
              this.reduForm.expiryEndTime = res.data.expiryEndTime;
              this.reduForm.smallProgram = res.data.smallProgram + '';
              this.reduForm.startEnd = [
              ...
              marking
                .manJianDetailCouponId({
                  manjianId: this.$route.query.id
                })
                .then(res => {
                  let _this = this;
                  if (res.code == 1) {
