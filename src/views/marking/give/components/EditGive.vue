  ...
  <div class="container">
    <div class="redu-box">
      ...
      <div class="redu-inset">
        <div class="currentTitle">满赠礼/<label> 编辑满赠礼</label></div>
        <Card>
          <Form :label-width="100" ref="reduForm" :model="reduForm" :rules="reduRule"
            ...
            <div class="inset-content">
              ...
              <FormItem label="优惠设置">
                <div v-for="(item,index) in customized_descs2" :key="index">
                  <div class="rule-box">
                    <!-- 左边 -->
  ...
  export default {
    ...
    methods: {
      ...
      listentgoodDatafucZp(data, index) {
        ...
        this.customized_descs2[index].gifts = rulegoodslist
      },
      //接收商品数据
      ...
      fullDetail() {
        let containerData = []
        let _this = this;
        marking.fullGiftDetailGiftGift({
            fullGiftId: this.$route.query.id
          })
          .then(res => {
            if (res.code == 1) {
              let arr = []
                _this.customized_descs2 = []
                res.data.map((v, i) => {
                  //满赠                                    
                  ...
                  if (v.rows3.rows2.conditions > 0) {
                    ...
                    goods.selGoodsIds({
                      ...
                      console.log(_this.customized_descs2)

                      console.log('123123123123123444',_this.customized_descs2)

                      let newCustomized = _this.sortByKey(_this.customized_descs2,'fullLevel')
      ...
      showToggle2(index) {
        if (this.customized_descs2[index].btnText == "或") {
          this.customized_descs2[index].btnText = "且";
          this.customized_descs2[index].conditions = 1;
        } else {
          this.customized_descs2[index].btnText = "或";
          this.customized_descs2[index].conditions = 2;
        }
        this.$forceUpdate()
      },
      // changeBox(e) {
      ...
      changeBox2(index, e) {
        ...
        this.customized_descs2[index].checkAllGroup = e
        // 检查单个对象的数组是否选满
        this.checkCusAry2();
      },
      ...
      deleteCusAry2(index) {
        this.customized_descs2.splice(index, 1)
      },
      ...
      checkCusAry2() {
        let newCusAry = [];
        this.customized_descs2.map((v, i) => {
          if (v.checkAllGroup.length == 2) {
            if (v.conditions == 1) {
              this.customized_descs2[i].conditions = 1;
              this.customized_descs2[i].btnText = "且";
            } else {
              this.customized_descs2[i].conditions = 2;
              this.customized_descs2[i].btnText = "或";
            }
            this.customized_descs2[i].showPrise = true;
            this.customized_descs2[i].flags = true;
          } else {
            this.customized_descs2[i].btnText = "";
            this.customized_descs2[i].showPrise = false;
            this.customized_descs2[i].flags = false;
            this.customized_descs2[i].conditions = 0;
          }
          newCusAry.push(this.customized_descs2[i]);
        });
        this.customized_descs2 = newCusAry;
      },

      //明细数据接口
      editGroup() {
        let _this = this;
        marking.fullGiftDetail({
