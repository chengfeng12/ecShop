<template>
  <div id="template-message">
    <div class="currentTitle">模板消息</div>
    <Card>
      <div class="header">
        <Button type="primary" v-if="hasRole(50109)" size="small" @click="add" class="mar-ri">+添加新模板</Button>
        <Input v-model="title" size="small" placeholder="输入模板标题搜索" style="width: 220px;" class="mar-ri"/>
      ...
      <div class="footer">
        <div class="operation">
          <Button v-if="hasRole(5010902)" type="default" size="small" class="lower" @click="batchStatus()">启用</Button>
          <Button v-if="hasRole(5010902)" type="default" size="small" class="del" @click="batchProhibit()">禁用</Button>
          <!-- @click="removeGoodsMui('批量删除')" -->
          <Button v-if="hasRole(5010903)" type="error" size="small" class="del" @click="batchDel()">删除</Button>
        </div>
        <div class="page">
          <Page
            :current="current"
...
export default {
                ...
                h(
                  "div",
                  {
                    ...
                    style: {
                      ...
                      paddingRight: "10px",
                      cursor: "pointer",
                      'display': this.hasRole(5010903) ?'block':'none',
                    },
                    on: {
                      click: () => {
                ...
                h(
                  "div",
                  {
                    ...
                    style: {
                      ...
                      padding: "0 10px",
                      cursor: "pointer",
                      'display': this.hasRole(5010902) ?'block':'none',
                    },
                    on: {
                      click: () => {
  ...
  methods: {
    ...
    editFun(id) {
      this.$router.push(
        "/@/views/shop/templatemessage/TemplateMessageAdd?id=" + id + "&role=2"
      );
    },
    statusFun(index) {
    ...
    delOkFun() {
      console.log('模板的',this.tempId)
      // return false
      shop
        .shopTemplateMessageDel({
          ids: this.tempId
        })
        .then(res => {
          if (res.code == "1") {
