<template>
  <div class="issuing-voucher">
    <div class="currentTitle">购物发券</div>
    <Card>
      <div class="item1" slot="title">
        <!-- 新增限时折扣 -->
        <Button type="primary" v-if="hasRole(4030401)" class="comBtn" @click="add" style="margin-left:0;">+添加新活动</Button>
      </div>
      <div class="wrapper">
        <div class="item2">
...
export default {
    ...
    return {
      ...
      columns7: [
...

        {
          ...
          render: (h,params) => {
            let text = null
            if (params.row.scope_id == 1) {
              text = '全部商品'
            } else if (params.row.scope_id == 2) {
              text = '指定商品'
            } else if (params.row.scope_id == 3) {
              text = '商品分类'
            } else if (params.row.scope_id == 4) {
              text = '商品分组'
            }
            return h('p',{},text)
          }
        ...
        },
        {
          ...
          render: (h,params) => {
              ...
              h("div", {
                  class: {},
                  style: {
                    borderRight: this.hasRole(4031101)? "1px solid #0083b0":"none",
                    paddingRight:"10px",
                    cursor: "pointer",
                    'display': this.hasRole(4030401) ?'block':'none',
                  },
                  on: {
                      click: () => {
              ...
              h("div", {
                  ...
                  style: {
                    ...
                    padding:"0 10px",
                    cursor: "pointer",
                    'display': this.hasRole(4031101) ?'block':'none',
                  },
                  on: {
                      click: () => {
                          this.statusFun(params.index);
              ...
              h("div", {
                  ...
                  style: {
                    padding:"0 10px",
                    cursor: "pointer",
                    'display': this.hasRole(4031102) ?'block':'none',
                  },
                  on: {
                      click: () => {
  ...
  methods: {
    ...
    add() {
      this.$router.push('/@/views/coupon/shopvoucher/components/AddShop?role=1')
    },
    editFun(id,index) {
      let status = this.tableData[index].activities_open
      if (status == 2) {
        this.$router.push('/@/views/coupon/shopvoucher/components/AddShop?id=' + id + '&role=2')
      } else {
    ...
    atatusOk() {
      marking.shoppingVouchersStatus({
        'id': this.tempId,
